...
//
// When mirror is set, the ramp fills only half the axis and is reflected about
// the centre, so the full colour sequence appears symmetrically on each side
// (outer edges = first colour, centre = last). The ramp is non-cyclic in that
// case. Used to break a face's material at its vertical centre instead of
// stretching one continuous ramp edge-to-edge.
